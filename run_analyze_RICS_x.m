function [Soln,Gs] = run_analyze_RICS_x(filename,data_ch,genotype,w0,wz,varargin)
...
%		has a different intensity than the cytoplasm.  This is according
...
if ischar(filename) && ~isfolder(filename)
	filenames = {filename};
...
%
% Looping through and analyzing each file.
%
...
for ii = 1:nLSM
	...
	try
		...
		%
		Gs_data = analyze_RICS_x(filename,data_ch,genotype,w0,wz,...
			sws,mask_ch,usemask,zerospad,hasbackground,[],yesplot);
		data = fit_both_models_2step(Gs_data,B0,D20,W0);


		% if yesplot
		% 	plot_RICS(Gs_data,data);
		% 
			% 		% 	if S_factor > 1
			% 		% 		[I_out,N_out,B_out,n_out,ep_out,V_out] = plot_NB(Gs_data,S_factor,sigma0,false);
			% 		% 	else
		% 	try
			% 	[I_out,N_out,B_out,n_out,ep_out,V_out] = plot_NB(Gs_data,data,sigma0,false);
		% 	catch
			% 	I_out = NaN; N_out = NaN; B_out = NaN; n_out = NaN; ep_out = NaN; V_out = NaN;
		% 	end
		% 
			% 		% 	end
		% 	Gs_data.I_nb = I_out;
		...
		Gs(j,1) = Gs_data;
		Soln(j,1) = data;
		disp(['j = ',num2str(j),' out of ',num2str(nLSM),' embryos'])
		j = j + 1;
		...
		save(['run_analyze_RICS_Error',outfilename],'LE','FE')
		disp(['e = ',num2str(e)])
		e = e + 1;
	end
...
matfilename1 = ['Mat/',clk,'_',outfilename,'fliprect_Soln'];
matfilename2 = ['Mat/',clk,'_',outfilename,'fliprect_Gs'];
save(matfilename1,'Soln')
save(matfilename2,'Soln','Gs','-v7.3')
