...
id = 'MATLAB:singularMatrix'; % temporarily turning off warning. See below
id2 = 'MATLAB:nearlySingularMatrix';
...
% "data"
%
fnames = fieldnames(Gs_data);
vG = contains(fnames,'Gs'); % exclude the big ACFs from the data
vM = contains(fnames,'mask'); % exclude the nuc and cyt masks
vI = strcmp(fnames,'Intensity'); % exclude the intensity array
vV = strcmp(fnames,'Variance'); % exclude the variance array
vtot = vG | vM | vI | vV;
fnames(vtot) = [];
for i = 1:length(fnames)
	data.(fnames{i}) = Gs_data.(fnames{i});
end
...
for i = idx
	...
	if strcmp(append{i},'cc')
		...
		%
		data = fit_immobile_CCF(GS,data,0,W0);
		...
		if ~any(isnan(GS(:)))
			...
			for j = 1:ngroups
				...
				% These lines are for the fitting eta-only case:
				xilimit2 = round(w01(j)/dr);
				Gs1 = Gs(:,1:xilimit2+1);
				Gs1 = Gs1(:); Gs1(1) = [];
				XI = {0:xilimit2 eta};
				...
				if exist('D20','var') && isnumeric(D20) && ~isempty(D20) && length(D20) == 1
					D201 = D20;
					DU = D20*(1 + 1e-4);
					DL = D20*(1 - 1e-4);
				elseif exist('D20','var') && isnumeric(D20) && ~isempty(D20) && length(D20) == 2
					D201 = mean(D20);
					DU = max(D20);
					DL = min(D20);
				elseif exist('D20','var') && isnumeric(D20) && ~isempty(D20) && length(D20) == 3
					D201 = D20(2);
					DU = D20(3);
					DL = D20(1);
				else
				...
				if ~isempty(J2)
					...
					warning('off',id)
					warning('off',id2)
					C = (J2'*J2) \ eye(length(p)); % we often get the warning
					% here. I think it's because we might be hitting a bound in
					% our parameters.
					warning('on',id)
					warning('on',id2)
