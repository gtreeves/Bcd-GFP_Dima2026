...
%
% The frame time can be gleaned from multiple places, and it's unclear if
% they are all equally accurate. Here we are storing one of these values in
% "lsminf2.TimeStamps.AvgStep".
%
% 1. cziinf.T.Interval.Increment
%	This seems like it would be the difference in the start time between
%	two frames, which would include resting time, but I have not investigated
%	that. This is also the value it takes on from organize_cziinf. There
%	must have been a reason I decided to overwrite that (I can't remember)
%	with tauf (below).
% 2. cziinfo.Frametime (from cziinf.Channel.LaserScanInfo.FrameTime)
%	I think this is probably the most accurate for how long it tames to
%	acquire one frame (not including the resting time). However, this field
%	does not exist in cziinf for line scans. (Instead, the field "LineTime"
%	exists in cziinf.Channel.LaserScanInfo)
% 3. tauf (from simple_czimetadata, which computes diff(timestamps)
%	This one might be the jankiest of all, because it comes from a weird
%	computation inside of simple_czimetadata.
%
% lsminf2.TimeStamps.AvgStep = cziinfo.Frametime;
...
if numSeries == 1 || FramesPerIteration == 1 || T == 1 || ...
		(FramesPerIteration == T && length(istart) < 2)
	IM = cell(numSeries,1);
	RICS_tiles = false;
