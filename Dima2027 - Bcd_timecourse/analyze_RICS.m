...
%		If this is not specified, but you still want to specify other 
%		arguments, put empty brackets -- [] -- in place of this argument.
%	* "sigma0": The standard deviation of the background levels (dark
%		image), needed for N&B analysis (see Dalal et a., 2008). Default,
%		zero.
%		If this is not specified, but you still want to specify other 
%		arguments, put empty brackets -- [] -- in place of this argument.
%	* "bleachcorrect_NB": Logical flag if you want to correct for bleaching
%		when calculating the Intensity and Variance frames for NB. This is
%		mutually exclusive with, and takes precedence over, the below. Default,
%		false.
%		If this is not specified, but you still want to specify other 
%		arguments, put empty brackets -- [] -- in place of this argument.
%	* "zerosatcorrect_NB": Logical flag if you want to correct for zero
%		saturation when calculating the Intensity and Variance frames for
%		NB. This is mutually exclusive with, and is overwritten by, the
%		above. Default, false. Default, false.
%		If this is not specified, but you still want to specify other 
%		arguments, put empty brackets -- [] -- in place of this argument.
%
%
...
if nArg >= iArg && ~isempty(varargin{iArg})
	...
	bg_lvl = true; % subtract background within ftn_imread (calls ftn_calcbg)
end, iArg = iArg + 1;
if nArg >= iArg && ~isempty(varargin{iArg})
 	sigma0 = varargin{iArg}; else
 	sigma0 = NaN;
end, iArg = iArg + 1;
if nArg >= iArg && ~isempty(varargin{iArg})
 	bleachcorrect_NB = varargin{iArg}; else
 	bleachcorrect_NB = false;
end, iArg = iArg + 1;
if nArg >= iArg && ~isempty(varargin{iArg})
 	zerosatcorrect_NB = varargin{iArg}; else
 	zerosatcorrect_NB = false;
end%, iArg = iArg + 1;
...
[IM,lsminf1,lsminf2,bg_lvl,sig0] = ftn_imread(filename,bg_lvl,stabilize);
if isnan(sig0)
	sig0 = sigma0; % if a numeric bg_lvl is given, then sig0 is not 
	% calculated. Therefore, if sigma0 is also given numerically, then we
	% will take that as sig0 (the sqrt of the darknoise)
end
...
[Gstot,Imean_tot,Var_tot,~,Sstot] = make_ACF_data(I,false,subtravg,sliding_window_size);	
totsignal = mean(Imean_tot); totvar = mean(Var_tot);
if usemask
	[Gsnuc,Imean_nuc,Var_nuc,~,Ssnuc] = make_ACF_data(I,nucmask,subtravg,sliding_window_size);
	[Gscyt,Imean_cyt,Var_cyt,~,Sscyt] = make_ACF_data(I,cytmask,subtravg,sliding_window_size);
	...
	if num_ch > 1 && mask_ch ~= data_ch
		...
		[Gscc,~,~,~,Sscc] = make_ACF_data(I2,nucmask,subtravg,sliding_window_size);
		
		%
		% Nuclear channel only
		%
		I_nuc = double(squeeze(IM(:,:,mask_ch,:)));
		[Gsnuc_ch,Imean_nuc_ch,Var_nuc_ch,~,Ssnuc_ch] = make_ACF_data(I_nuc,nucmask,subtravg,sliding_window_size);
...
% course (for N+B). This includes correction for bleaching
%
I0 = double(squeeze(IM(:,:,data_ch,:)));

if bleachcorrect_NB
	%
	% To correct for bleaching, we will calculate the bleaching trend by
	% running a bi-directional exponential filter. The filter captures the
	% trend but keeps the high frequency variation. Then we subtract off 
	% this trend and add back in the initial value of the trend, which
	% restores intensity to its "pre-bleach" value while keeping the
	% variation.
	%
	alpha = 0.1; % approx equal to a time const of 10 frames: 1-exp(-dt/tau)
	% where "dt" is one frame (ie, the difference between two frames, in units
	% of frames)

	% forward exponential filter
	I2 = zeros(H,W,num_frames);
	I2(:,:,1) = I0(:,:,1);
	for i = 2:num_frames
		I2(:,:,i) = alpha*I0(:,:,i) + (1-alpha)*I2(:,:,i-1);
	end

	% backward exponential filter (to maintain phase)
	I3 = zeros(H,W,num_frames);
	I3(:,:,end) = I2(:,:,end);
	for i = num_frames-1:-1:1
		I3(:,:,i) = alpha*I2(:,:,i) + (1-alpha)*I3(:,:,i+1);
	end

	% applying filter by subtracting off trend, then adding back in initial
	% point of filtered data. We do that instead of adding back in the average
	% across the time course because it maintains the "pre-bleach" intensity
	I1 = I0 - I3 + I3(:,:,1);
elseif zerosatcorrect_NB
	%
	% To correct for zero saturation, we need (1) fewer than 50% of pixels to
	% be zero-saturated & (2) the histogram of intensities to be otherwise
	% Gaussian. We will calculate the mean and variance of the part that
	% is above the zero-saturation (technically, greater than one bg_std
	% above zero) and use that.
	%
	bg_std = Gs.metadata.std_bg(data_ch);
	v = I0 > bg_std;
	I01 = median(I0,3);

	NUC = all(nucmask,3);
	A = zeros(sum(NUC(:)),num_frames);
	for i = 1:num_frames, I01 = I0(:,:,i); A(:,i) = I01(NUC); end
	A1 = A(:);
	[N,edges] = histcounts(A1);
	v = A1 < 3*Gs.metadata.std_bg(1);
	B = A1(~v);
	...
	[N,edges] = histcounts(A1,'Normalization','pdf');
	x = edges(1:end-1)';
	px = N';
	[phi0,mu0,s0] = GMM_histalready([x,px],2);
	[phi,mu,s,R2,delta68,delta95] = GMM_histlsq([x,px],2,phi0,[-10 mu0(2)/2;10 mu0(2)*2],s0);
	f = ftn_multigauss([phi mu s],x);
	figure
	plot(x,px)
	hold on
	plot(x,f)
