function [Gs_out,Imean_out,Var_out,Gs_t,Ss_out] = make_ACF_data(IM,varargin)
...
%		If this is not specified, but you still want to specify other 
%		arguments, put empty brackets -- [] -- in place of this argument.
%	* "Grpidx": Could be a scalar or vector. If a scalar, and if no mask is
%		being used, then this will be used as "ngroups": the number of
%		groups to bin the average intensities and time-varying ACFs into at
%		the end of the function. If a vector, it represents the vector of
%		group indices. Allows us to track which frames belog to which mask
%		group, and will also be used to average frames together for time
%		course purposes. Default, empty.
%
...
%
% Outputs:
%	"Gs_out": The 2D autocorrelation matrix. It is calculated by averaging
%		the ACF for each timepoint, because usually the ACFs from single
%		time points are too noisy and you need the averaging to get a
%		smooth curve. So, we average all the ACFs together across all time
%		points.
...
%	"Gs_t": the timecourse of the ACF, if there are enough frames and
%		pixels to break down the calculation into different time points
%	"Ss_out": as explained above, the output ACF is the average of ACFs
%		from individual time points. This output is the standard deviation
%		of the ACFs from all time points.
%	
...
Var_out = squeeze(Gs(1,1,:)).*(Imean_out.^2); % variance of each frame, 
% including the shot noise, but not the shape variance
...
%
if ~isempty(Grpidx) && length(Grpidx) == T
	ngroups = max(Grpidx); % Intention: this was passed because of RICS_tiles
else
	if ~isempty(Grpidx)
		ngroups = max(Grpidx); 
	else
		ngroups = 10; % default value if it wasn't stored/passed in "Grpidx"
	end
	nslices = floor(T/ngroups); 
	for i = 1:ngroups
