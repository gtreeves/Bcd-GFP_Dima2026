...
elseif strcmp(filename(end-2:end),'czi')
	...
	try % redundancy because sometimes it looks like bfgetreader just 
		% randomly fails, so just try again
...
if exist('subtr_bg','var') && ~isempty(subtr_bg)
	...
	elseif islogical(subtr_bg) && subtr_bg
		...
		for i = 1:length(IM)
			IM1 = IM{i};
			[bg,bgsig] = ftn_calcbg(IM1(:,:,:,1,1));
			for o = 1:num_ch
				% IM1(:,:,o,:) = imsubtract(IM1(:,:,o,:,:),bg(o));
				IM1(:,:,o,:,:) = imsubtract(IM1(:,:,o,:,:),bg(o)); % should have five dimensions?
			end
			IM{i} = IM1;
		end
	else
